...
const page = ({ session }) => {
  ...
  // Extracting permissions for the "session" submenu
  const academicMenu = menuData
    ? menuData.find((menu) => menu.name === "Academic")
    : null;
  const sessionSubMenu = academicMenu?.sub_menu
    ? academicMenu.sub_menu.find((subMenu) => subMenu.name === "Session")
    : null;
  const sessionPermissions = sessionSubMenu?.permission || [];
  ...
  return (
    <Box>
      ...
      >
        ...
        {sessionPermissions && sessionPermissions.includes("create") ? (
          <Button
            startIcon={<AddIcon />}
            variant="contained"
            size="small"
        ...
        >
          <Box sx={style}>
            <SessionModal handleClose={handleClose} accessToken={accessToken} />
          </Box>
        </Modal>
        <Backdrop
