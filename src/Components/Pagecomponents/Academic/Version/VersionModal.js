...
const VersionModal = ({ handleClose, accessToken }) => {
  ...
  const { setVersionSaved } = useVersionContext();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (responseData.code === 200) {
        handleClose();
        setVersionSaved();
        toast.success(`Successfully ${versionName} version Saved`, {
          position: "top-right",
          autoClose: 3000,
        ...
        toast.error(`${versionName} already exists`, {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
