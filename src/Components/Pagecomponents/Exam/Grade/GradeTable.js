...
const columns = [
  ...
  { id: "grade", label: "Grade", align: "center", sortable: true },
  { id: "startmark", label: "Start Mark", align: "center", sortable: true },
  { id: "endmark", label: "End Mark", align: "center", sortable: true },
  { id: "point", label: "Point", align: "center", sortable: true },
  {
    id: "serialno",
    label: "Serial No",
    align: "center",
...
const GradeTable = ({ session }) => {
  ...
  const { isGradeSaved, resetGradeSaved } = useGradeContext();
  ...
  const [beforeGradeName, setBeforeGradeName] = useState(null);
  ...
  useEffect(() => {
    ...
    // const accessToken = "sds"
    const accessToken = session?.user?.data?.token?.access;
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/exam/api/grade?page_number=${
        page + 1
  ...
  }, [deleteTrigered, page, rowsPerPage, editingRow, isGradeSaved]);

  const rows = GradeData?.data.map((item, index) => {
    const permissions = GradePermissions || [];
  ...
  const handleDelete = (id, deletedGradeName) => {
    swalWithMuiButtons
      .fire({
        title: "Are you sure?",
        html: `You won't be able to revert <b>${deletedGradeName}!</b>`,
        icon: "warning",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(`${process.env.NEXT_PUBLIC_HOST}/exam/api/grade/delete/${id}`, {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${accessToken}`,
            ...
            .then((responseData) => {
              if (responseData.code == 200) {
                const filterdData = GradeData.data.filter(
                  (Grade) => Grade.id !== id
                );
                setGradeData({ ...GradeData, data: filterdData });
                setDeleteTrigered(true);
                swalWithMuiButtons.fire(
                  "Deleted!",
                  `Grade <b>${deletedGradeName}</b> has been deleted.`,
                  "success"
                );
              }
              if (responseData.code == 401) {
  ...
  const handleEdit = (rowids, GradeName) => {
    setEditingRow(rowids);
    setEditedGradeName(GradeName);
    setBeforeGradeName(GradeName);
  };
  ...
  const filteredRows = (sortedRows || []).filter((row) => {
    return row.grade.toLowerCase().includes(searchText.toLowerCase());
  });
                              ...
                              {editingRow === row.ids ? (
                                ...
                                <>
                                  ...
                                  row.permissions.includes("delete") ? (
                                    <IconButton
                                      onClick={(e) =>
                                        handleDelete(row.ids, row.grade)
                                      }
                                    >
                                      <Icon
                                        className="text-rose-700"
