...
const FeesTypeModal = ({
  ...
  const {
    ...
    defaultValues: {
      name: editingRow && editingRow ? editingRow?.name : "",
      description: editingRow && editingRow ? editingRow?.description : "",
    },
    ...
    try {
      if (editingRow) {
        response = await fetch(
          `${process.env.NEXT_PUBLIC_HOST}/fees/api/fees-type/detail/${editingRow.id}`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${accessToken}`,
        ...
        response = await fetch(
          `${process.env.NEXT_PUBLIC_HOST}/fees/api/fees-type`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${accessToken}`,
      ...
      if (responseData.code == 200) {
        handleClose();
        // setSubjectSaved();
        toast.success(`Successfully Add Fees Type`, {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
        ...
        >
          <Grid container spacing={2}>
            ...
            {/* <Grid item xs={12} md={6}>
              ...
              >
                <Box sx={{ display: "flex", flexDirection: "row", gap: "3px" }}>
                  <Typography sx={{ ml: "15px" }}>Code</Typography>
                  <Typography sx={{ color: "#786CF1", fontSize: "20px" }}>
                    *
                  </Typography>
                </Box>
