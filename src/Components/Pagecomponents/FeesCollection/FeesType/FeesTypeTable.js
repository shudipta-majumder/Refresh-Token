...
import FeesTypeModal from "./FeesTypeModal";
...
const FeesTypeTable = ({ session }) => {
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/fees/api/fees-type?page_number=${
        page + 1
    ...
    swalWithMuiButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(
            `${process.env.NEXT_PUBLIC_HOST}/fees/api/fees-type/delete/${id}`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${accessToken}`,
