...
const columns = [
  ...
  { id: "Bankname", label: "Bank Name", align: "center", sortable: true },
...
const BankTable = ({ session }) => {
  ...
  const { isBankSaved, resetBankSaved } = useBankContext();
  ...
  const [beforeBankName, setBeforeBankName] = useState(null);
  ...
  const academicMenu = menuData
    ...
    : null;
  const BankSubMenu = academicMenu?.sub_menu
    ? academicMenu.sub_menu.find((subMenu) => subMenu.name === "Bank")
    : null;
  const BankPermissions = BankSubMenu?.permission || [];
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/hrms/api/bank?page_number=${
        page + 1
  ...
  }, [deleteTrigered, page, rowsPerPage, editingRow, isBankSaved]);

  const rows = BankData?.data.map((item, index) => {
    const permissions = BankPermissions || [];
  ...
  const handleDelete = (id, deletedBankName) => {
    swalWithMuiButtons
      .fire({
        title: "Are you sure?",
        html: `You won't be able to revert <b>${deletedBankName}!</b>`,
        icon: "warning",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(`${process.env.NEXT_PUBLIC_HOST}/hrms/api/bank/delete/${id}`, {
            method: "PATCH",
            headers: {
              Authorization: `Bearer ${accessToken}`,
            ...
            .then((responseData) => {
              if (responseData.code == 200) {
                const filterdData = BankData.data.filter(
                  (Bank) => Bank.id !== id
                );
                setBankData({ ...BankData, data: filterdData });
                setDeleteTrigered(true);
                swalWithMuiButtons.fire(
                  "Deleted!",
                  `Bank <b>${deletedBankName}</b> has been deleted.`,
                  "success"
                );
              }
              if (responseData.code == 401) {
  ...
  const filteredRows = (sortedRows || []).filter((row) => {
    return row.Bankname.toLowerCase().includes(searchText.toLowerCase());
  });
                              ...
                              {editingRow === row.ids ? (
                                ...
                                <>
                                  ...
                                  row.permissions.includes("delete") ? (
                                    <IconButton
                                      onClick={(e) =>
                                        handleDelete(row.ids, row.Bankname)
                                      }
                                    >
                                      <Icon
                                        className="text-rose-700"
