...
const columns = [
  ...
  { id: "countrycode", label: "Country Code", align: "center", sortable: true },
  { id: "Countryname", label: "Country Name", align: "center", sortable: true },
...
const CountryTable = ({ session }) => {
  ...
  const { isCountrySaved, resetCountrySaved } = useCountryContext();
  ...
  const [beforeCountryName, setBeforeCountryName] = useState(null);
  ...
  const academicMenu = menuData
    ...
    : null;
  const CountrySubMenu = academicMenu?.sub_menu
    ? academicMenu.sub_menu.find((subMenu) => subMenu.name === "Country")
    : null;
  const CountryPermissions = CountrySubMenu?.permission || [];
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/setup/api/country?page_number=${
        page + 1
  ...
  }, [deleteTrigered, page, rowsPerPage, editingRow, isCountrySaved]);

  const rows = CountryData?.data.map((item, index) => {
    const permissions = CountryPermissions || [];
  ...
  const handleDelete = (id, deletedCountryName) => {
    swalWithMuiButtons
      .fire({
        title: "Are you sure?",
        html: `You won't be able to revert <b>${deletedCountryName}!</b>`,
        icon: "warning",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(
            `${process.env.NEXT_PUBLIC_HOST}/setup/api/country/delete/${id}`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${accessToken}`,
            ...
            .then((responseData) => {
              if (responseData.code == 200) {
                const filterdData = CountryData.data.filter(
                  (Country) => Country.id !== id
                );
                setCountryData({ ...CountryData, data: filterdData });
                setDeleteTrigered(true);
                swalWithMuiButtons.fire(
                  "Deleted!",
                  `Country <b>${deletedCountryName}</b> has been deleted.`,
                  "success"
                );
              }
              if (responseData.code == 401) {
  ...
  const handleEdit = (rowids, CountryName) => {
    setEditingRow(rowids);
    setEditedCountryName(CountryName);
    setBeforeCountryName(CountryName);
  };

  const handleSave = (rowids) => {
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/staff/api/Country/detail/${rowids}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${accessToken}`,
      ...
      .then((responseData) => {
        if (responseData.code == 200) {
          const updatedData = CountryData.data.map((item) => {
            if (item.id === rowids) {
              return { ...item, Countryname: editedCountryName };
            }
            return item;
          });
          setCountryData({ ...CountryData, data: updatedData });
          setEditingRow(null);
          setEditedCountryName("");
          router.refresh();
          toast.success(
            `Successfully Country ${beforeCountryName} to ${editedCountryName} Updated`,
            {
              position: "top-right",
              autoClose: 3000,
              hideProgressBar: false,
        ...
        if (responseData.code == 400) {
          toast.error(`Country ${editedCountryName} already exits`, {
            position: "top-right",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
  ...
  const filteredRows = (sortedRows || []).filter((row) => {
    return row.Countryname.toLowerCase().includes(searchText.toLowerCase());
  });
                              ...
                              {editingRow === row.ids ? (
                                ...
                                <>
                                  ...
                                  row.permissions.includes("delete") ? (
                                    <IconButton
                                      onClick={(e) =>
                                        handleDelete(row.ids, row.Countryname)
                                      }
                                    >
                                      <Icon
                                        className="text-rose-700"
