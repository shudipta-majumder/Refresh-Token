...
const DistrictTable = ({ session }) => {
  ...
  const { isDistrictSaved, resetDistrictSaved } = useDistrictContext();
  ...
  const [beforeDistrictName, setBeforeDistrictName] = useState(null);
  ...
  const academicMenu = menuData
    ...
    : null;
  const DistrictSubMenu = academicMenu?.sub_menu
    ? academicMenu.sub_menu.find((subMenu) => subMenu.name === "District")
    : null;
  const DistrictPermissions = DistrictSubMenu?.permission || [];
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/setup/api/district?page_number=${
        page + 1
  ...
  }, [deleteTrigered, page, rowsPerPage, editingRow, isDistrictSaved]);

  const rows = DistrictData?.data.map((item, index) => {
    const permissions = DistrictPermissions || [];
  ...
  const handleDelete = (id, deletedDistrictName) => {
    swalWithMuiButtons
      .fire({
        title: "Are you sure?",
        html: `You won't be able to revert <b>${deletedDistrictName}!</b>`,
        icon: "warning",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(
            `${process.env.NEXT_PUBLIC_HOST}/setup/api/district/delete/${id}`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${accessToken}`,
            ...
            .then((responseData) => {
              if (responseData.code == 200) {
                const filterdData = DistrictData.data.filter(
                  (District) => District.id !== id
                );
                setDistrictData({ ...DistrictData, data: filterdData });
                setDeleteTrigered(true);
                swalWithMuiButtons.fire(
                  "Deleted!",
                  `District <b>${deletedDistrictName}</b> has been deleted.`,
                  "success"
                );
              }
              if (responseData.code == 401) {
  ...
  const handleEdit = (rowids, DistrictName) => {
    setEditingRow(rowids);
    setEditedDistrictName(DistrictName);
    setBeforeDistrictName(DistrictName);
  };
  ...
  const filteredRows = (sortedRows || []).filter((row) => {
    return row.Districtname.toLowerCase().includes(searchText.toLowerCase());
  });
                              ...
                              {editingRow === row.ids ? (
                                ...
                                <>
                                  ...
                                  row.permissions.includes("delete") ? (
                                    <IconButton
                                      onClick={(e) =>
                                        handleDelete(row.ids, row.Districtname)
                                      }
                                    >
                                      <Icon
                                        className="text-rose-700"
