...
const DivisionTable = ({ session }) => {
  ...
  const { isDivisionSaved, resetDivisionSaved } = useDivisionContext();
  ...
  const [beforeDivisionName, setBeforeDivisionName] = useState(null);
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/setup/api/division?page_number=${
        page + 1
  ...
  }, [deleteTrigered, page, rowsPerPage, editingRow, isDivisionSaved]);

  const rows = DivisionData?.data.map((item, index) => {
    const permissions = DivisionPermissions || [];
  ...
  const handleDelete = (id, deletedDivisionName) => {
    swalWithMuiButtons
      .fire({
        title: "Are you sure?",
        html: `You won't be able to revert <b>${deletedDivisionName}!</b>`,
        icon: "warning",
      ...
      .then((result) => {
        if (result.isConfirmed) {
          fetch(
            `${process.env.NEXT_PUBLIC_HOST}/setup/api/division/delete/${id}`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${accessToken}`,
            ...
            .then((responseData) => {
              if (responseData.code == 200) {
                const filterdData = DivisionData.data.filter(
                  (Division) => Division.id !== id
                );
                setDivisionData({ ...DivisionData, data: filterdData });
                setDeleteTrigered(true);
                swalWithMuiButtons.fire(
                  "Deleted!",
                  `Division <b>${deletedDivisionName}</b> has been deleted.`,
                  "success"
                );
              }
              if (responseData.code == 401) {
  ...
  const handleCancelEdit = () => {
    setEditingRow(null);
    setEditedDivisionName("");
  };

  const filteredRows = (sortedRows || []).filter((row) => {
    return row.Divisionname.toLowerCase().includes(searchText.toLowerCase());
  });
                          ...
                          <StyledTableCell key={column.id} align={column.align}>
                            ...
                            editingRow === row.ids ? (
                              <TextField
                                ...
                                onChange={(e) =>
                                  setEditedDivisionName(e.target.value)
                                }
                                sx={{
                                  border: "none",
                                  "& input": {
