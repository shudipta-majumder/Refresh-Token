...
const CalendarModal = ({ accessToken, handleClose, updateEvent, holidayTypes, handleDelete }) => {
  const { color, colorX } = useOnlyIcon();
  const { setCalendarSaved } = useCalendarContext();
  const [onChangedHolidayType, setOnchangedHolidayType] = useState(updateEvent?.type === 1 ? 'holiday' : 'event');
  ...
  const { control, handleSubmit, setValue, reset } = useForm({
    defaultValues: {
      ...
      start_date: updateEvent ? dayjs(`${updateEvent?.start_date}T${updateEvent?.start_time ? updateEvent?.start_time : '00:00:00'}`) : null,
      end_date: updateEvent ? dayjs(`${updateEvent?.end_date}T${updateEvent?.end_time ? updateEvent?.end_time : '23:59:59'}`) : null,
    },
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await fetch(updateEvent && updateEvent.id ? putApi : postApi, {
  ...
  return (
    <section>
      <Box>
        ...
        >
          <Box component='form' noValidate autoComplete='off' onSubmit={handleSubmit(onSubmit)} sx={{ textAlign: 'center' }}>
            {updateEvent && updateEvent?.id ? (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Box component={'h3'} sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                  Update Event
                </Box>
                <Box>
                  <IconButton onClick={() => handleDelete(updateEvent?.id)}>
                    <Icon title='Delete' className='text-rose-700' icon='uiw:delete' />
                  </IconButton>
                </Box>
              </Box>
            ) : (
              <Box component={'h3'}>Add Event</Box>
            )}
            ...
            <Grid container spacing={2}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  ...
                  <Controller
                    ...
                    render={({ field, fieldState: { error } }) => (
                      <TextField {...field} error={!!error} helperText={error?.message} fullWidth size='small' placeholder='Type Here' />
                    )}
                  />
                </Box>
              </Grid>
              ...
              <Grid item xs={12}>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <Box sx={{ display: 'flex', flexDirection: 'row' }}>
                    <Typography sx={{ ml: '15px' }}>Type</Typography>
                    <Typography sx={{ color: '#786CF1', fontSize: '20px' }}>*</Typography>
                  </Box>
                  <Controller
                    name='type'
                    control={control}
                    render={({ field, fieldState: { error } }) => (
                      <TextField select {...field} onChange={handleChange} error={!!error} helperText={error?.message} size='small' fullWidth>
                        {holidayTypes.length > 0 ? (
                          holidayTypes.map((holidayType) => (
                            <MenuItem key={holidayType.id} value={holidayType.id} sx={{ color: holidayType.code }}>
                              {holidayType.name}
                            </MenuItem>
                          ))
                        ) : (
                          <small className='text-red-500 px-6'>Server response was not ok!</small>
                        )}
                      </TextField>
                    )}
                  />
              ...
              >
                ...
                <Button
                  ...
                  size='small'
                  sx={{
                    background: 'linear-gradient(45deg,#B22222 70%, #C11B17 30% )',
                    ':hover': {
                      bgcolor: '#796EF1',
                    },
                    padding: '5px 30px',
