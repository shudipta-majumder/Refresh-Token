...
const Third = ({ PersonalDetails }) => {
  const { atten_daily } = PersonalDetails;
  const StaffID = PersonalDetails?.staff_id;

  const rows = atten_daily?.map((item, index) => {
