...
const columns = [
  ...
  { id: "thanacode", label: "Thana Code", align: "center", sortable: true },
  { id: "Thananame", label: "Thana Name", align: "center", sortable: true },
  {
    id: "districtname",
    label: "District Name",
    align: "center",
...
const ThanaTable = ({ session }) => {
  ...
  const { isThanaSaved, resetThanaSaved } = useThanaContext();
  ...
  const ThanaSubMenu = academicMenu?.sub_menu
    ? academicMenu.sub_menu.find((subMenu) => subMenu.name === "Thana")
    : null;
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_HOST}/setup/api/thana?page_number=${
        page + 1
  ...
  }, [deleteTrigered, page, rowsPerPage, editingRow, isThanaSaved]);
  ...
  const handleSave = (rowids) => {
    fetch(`${process.env.NEXT_PUBLIC_HOST}/staff/api/Thana/detail/${rowids}`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ name: editedThanaName }),
    })
      .then((response) => {
        return response.json();
      })
      .then((responseData) => {
        if (responseData.code == 200) {
          const updatedData = ThanaData.data.map((item) => {
            if (item.id === rowids) {
              return { ...item, Thananame: editedThanaName };
            }
            return item;
          });
          setThanaData({ ...ThanaData, data: updatedData });
          setEditingRow(null);
          setEditedThanaName("");
          router.refresh();
          toast.success(
            `Successfully Thana ${beforeThanaName} to ${editedThanaName} Updated`,
            {
              position: "top-right",
              autoClose: 3000,
              hideProgressBar: false,
        ...
        if (responseData.code == 400) {
          toast.error(`Thana ${editedThanaName} already exits`, {
            position: "top-right",
            autoClose: 3000,
            hideProgressBar: false,
            closeOnClick: true,
  ...
  const filteredRows = (sortedRows || []).filter((row) => {
    return row.Thananame.toLowerCase().includes(searchText.toLowerCase());
  });
