...
const studentLeaveCreateSchema = yup.object().shape({
  id: yup.string().required("This field may not be blank."),
  start_date: yup
    .date()
    .nullable()
...
const StudentLeaveApplicationCreate = ({
  handleAddModalClose,
  setAddFeeTypes,
  session,
  classTeacherWiseStudent,
}) => {
  ...
  const studentSearch = classTeacherWiseStudent?.data?.map(
    // (clsTecStd) => `${clsTecStd.first_name} ${clsTecStd.last_name}`
    (clsTecStd) => clsTecStd
  );
  ...
  const onSubmit = async (data) => {
    ...
    studentLeaveCreateData.append("apply_by", data.id);
    ...
    try {
      response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/student/api/leave/create`,
        {
          method: "POST",
          headers: {
      ...
      if (responseData.code == 200) {
        handleAddModalClose();
        // setSubjectSaved();
        toast.success(`Successfully Leave Created`, {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          closeOnClick: true,
          ...
          <Grid container spacing={2}>
            ...
            <Grid item md={12}>
              ...
              >
                <Box sx={{ display: "flex", flexDirection: "row", gap: "3px" }}>
                  <Typography sx={{ ml: "15px" }}>Put Student Id</Typography>
                  ...
                  render={({ field, fieldState: { error } }) => (
                    <Autocomplete
                      ...
                      onChange={(event, newValue) => {
                        ...
                        setStudentId(newValue ? newValue.id : null);
                      }}
            ...
            <Grid item sm={12} xs={12}>
              ...
              >
                ...
                <Icon className={classes.icon} icon="pepicons-pencil:file" />
                ...
                <Box>
                  {sectionRowData && sectionRowData.document ? (
                    <>
                      {pdfPreview ? (
                        <>
                          {pdfPick.type && (
...
export default StudentLeaveApplicationCreate;
