...
const StudentAdmission = ({
  ...
  const handleNext = async (data) => {
    ...
    if (responseId) {
      ...
      //     }/student/api/guardian/image/${responseData?.data?.guardians.map(
      //       (gid) => gid.id
      //     )}`,
      //     {
      //       method: "PATCH",
      ...
      //         Authorization: `Bearer ${accessToken}`,
      //       },
      //       body: guardianPhotoSend,
      //     }
      //   );
      // }

      // if (activeStep === 1 && guardianPhoto) {
      //   // Iterate over each guardian ID and make a separate API call for each
      //   responseData?.data?.guardians.map(async (guardian) => {
      //     guardianImageResponse = await fetch(
      //       `${process.env.NEXT_PUBLIC_HOST}/student/api/guardian/image/${guardian.id}`,
      //       {
      //         method: "PATCH",
      //         headers: {
      //           Authorization: `Bearer ${accessToken}`,
